...
const getSchemaShape = (inputSchema: boolean | JSONSchema7): Record<string, z.ZodTypeAny> => {
  const schemaShape: Record<string, z.ZodTypeAny> = {};

  if (inputSchema && typeof inputSchema === 'object' && 'properties' in inputSchema) {
    const properties = (inputSchema.properties ?? {}) as Record<string, JSONSchema7 | boolean>;
    const required = Array.isArray((inputSchema as JSONSchema7).required) ? ((inputSchema as JSONSchema7).required as string[]) : [];

    const mapPrimitive = (type: string | string[] | undefined, prop: JSONSchema7): z.ZodTypeAny | undefined => {
      ...
      switch (baseType) {
        ...
        case 'boolean':
          return z.boolean();
        case 'array': {
          const items = typeof prop.items === 'object' && prop.items ? prop.items : undefined;
          if (items && typeof items === 'object' && !Array.isArray(items)) {
            const itemType = mapPrimitive((items as JSONSchema7).type, items as JSONSchema7) || z.unknown();
    ...
    Object.entries(properties).forEach(([key, prop]) => {
      ...
      // Handle enums (prefer exact literals when possible)
      if (Array.isArray((prop as JSONSchema7).enum) && (prop as JSONSchema7).enum!.length > 0) {
        const enumVals = (prop as JSONSchema7).enum as Array<string | number>;
        ...
        if (allStrings && enumVals.length > 0) {
          // z.enum requires at least one string literal
      ...
      if (!zodField) {
        zodField = mapPrimitive((prop as JSONSchema7).type, prop as JSONSchema7);
      }

      if (!zodField) return; // Skip unsupported types

      if ((prop as JSONSchema7).description) {
        zodField = zodField.describe((prop as JSONSchema7).description as string);
            }
...
// For Vercel Sandbox - bind to all interfaces and use environment port
